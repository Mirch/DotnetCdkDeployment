...
exports.invoke = async (targetDirectory) => {
    const slnPath = path.join(targetDirectory, 'src', 'Infrastructure.sln');
    const csprojPath = path.join(targetDirectory, 'src', 'Infrastructure', 'Infrastructure.csproj');
    ...
    await new Promise((resolve, reject) => {
        ...
        child.once('exit', code => {
            if (code === 0) {
                ...
                reject(new Error(`Could not add project Infrastructure.csproj to solution Infrastructure.sln. Error code: ${code}`));
            }
        });
    });
};
